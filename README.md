...
- **单一默认接收人是 QQ 平台限制下的设计选择。** QQ 个人机器人不支持主动添加好友，也不能像普通 QQ 账号一样维护与多个用户的好友私聊关系；因此持久化存储只有一个 `data/recipient_openid`，任何新私聊都会覆盖此前的默认接收人。显式 `openid` 只是定向发送能力，不会保存用户或维护订阅关系。QQ 开放多用户好友私聊能力后，项目会补充多用户绑定与通知能力。
